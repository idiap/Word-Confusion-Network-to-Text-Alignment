...
"""
Some sections of this code were inspired by the paper from Liu, C., Zhu, S., Zhao, Z., Cao, R., Chen, L., & Yu, K. (2020). Jointly Encoding Word Confusion Network and Dialogue Context with BERT for Spoken Language Understanding.
"""
import copy
import math
...
def get_slf_attn_w(score_seq):
    _, lw = score_seq.size()
    attn_w = score_seq.unsqueeze(1).expand(-1, lw, -1)  # b x l x l
    return attn_w
...
class BertSelfAttention(nn.Module):
    def __init__(self, bert_config, bert_self_attention, n_heads, score_util):
        super().__init__()
        ...
        self.attention_head_size = int(
            bert_config.hidden_size / self.num_attention_heads
        )
        ...
        if self.score_util == "pp":
            self.score_lambda = nn.Parameter(
                torch.randn(1, self.num_attention_heads, 1, 1)
            )
    ...
    def forward(self, hidden_states, attention_mask, head_mask, attn_w):
        ...
        if attn_w is not None:
            if self.score_util == "pp":
                attn_w = attn_w.unsqueeze(1).expand(
...
class BertAttention(nn.Module):
    def __init__(self, bert_config, bert_attention, dp, score_util, n_heads):
        super().__init__()
        ...
        self.self = BertSelfAttention(
            bert_config, bert_self_attention, n_heads, score_util
        )
...
class BertLayer(nn.Module):
    def __init__(self, bert_config, bert_layer, dp, score_util, n_heads):
        super().__init__()
        ...
        self.attention = BertAttention(
            bert_config, bert_attention, dp, score_util, n_heads
        )

    def forward(self, hidden_states, attention_mask, head_mask, slf_attn_w):
        self_attention_outputs = self.attention(
            hidden_states, attention_mask, head_mask, slf_attn_w
        )
...
class BertEncoder(nn.Module):
    def __init__(
        self, bert_config, pretrained_model_opts, score_util, n_layers=12, n_heads=6
    ):
        super().__init__()
        ...
        self.layer_stack = nn.ModuleList(
            [
                BertLayer(bert_config, bert_layers[i], dp, score_util, n_heads)
    ...
    def forward(
        self, inputs, attention_mask=None, return_attns=False, default_pos=False
    ):
        ...
        for i, enc_layer in enumerate(self.layer_stack):
            enc_output, enc_slf_attn = enc_layer(
                enc_output, attention_mask, head_mask=None, slf_attn_w=slf_attn_w
            )
            if return_attns:
